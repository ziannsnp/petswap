...
// A location is only in-app when its path begins with a single slash followed by
// something that is not another separator. Allowing the second character to be '/'
// or '\' hands the browser an authority instead of a path: it reads '//example.com'
// as protocol-relative, and normalizes '/\example.com' into exactly that.
...
function redirectDestination(state: unknown): string {
  ...
  if (typeof from?.pathname !== 'string' || !IN_APP_PATH_PATTERN.test(from.pathname)) {
...
export function AuthScreen() {
  ...
  //
  // Only the client's own complaints are retracted. A server rejection outlives the
  // keystroke that follows it, because typing does not answer it: 'Confirm your email
  // address' and 'Too many attempts' stay true no matter what is in the fields, and
  // clearing them on edit left the user resubmitting into the same wall with nothing
  // on screen to explain it. Retiring them is the next submit's job, whether it
  // reaches the server or handleSubmit rejects it first.
